...
data class ContinuousEffect(
    ...
    val groupId: String? = null,
    /**
     * Whether this effect is generated by a *static ability* of its source (CR 611.3) rather than
     * by the resolution of a spell or ability (CR 611.2).
     *
     * The distinction is load-bearing for "loses all abilities": a static ability only generates
     * its effect for as long as the object actually has that ability (CR 611.3b), so stripping the
     * source's abilities in Layer 6 stops the effect. A continuous effect created by a *resolved*
     * spell or ability is independent of its source — it "lasts as long as stated by the spell or
     * ability creating it" (CR 611.2a) and keeps applying even if the source later loses all its
     * abilities, or leaves the battlefield entirely.
     *
