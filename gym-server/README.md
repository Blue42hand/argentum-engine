...
Spring Boot HTTP transport on top of [`:gym`](../gym/README.md)'s
...
don't need this module; use `:gym` and
[`:gym-trainer`](../gym-trainer/README.md) instead.

## Why this module exists

`:gym` is transport-agnostic on purpose. An HTTP shell is one of
...
- `:gym` stays a pure library that can be embedded in a JVM
...
`SnapshotHandle`, …) is already `@Serializable` in `:gym`, and
...
```bash
GYM_OPENAPI_ENABLED=false ./gradlew :gym-server:bootRun
```
...
## When to use this instead of `:gym-trainer`
...
- The trainer is JVM-side and embeds `:gym` directly → skip this
  module, use `:gym-trainer`.
