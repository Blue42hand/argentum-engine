...
class MultiEnvService(
     ...
     * is supplied, verify it before resolving/executing the action ID so a delayed request cannot
     * silently execute against a newer registry.
     */
    fun step(request: StepRequest): ObservationResult {
        val env = requireEnv(request.envId)
        ...
        return env.step(request.actionId, request.params)
    }
