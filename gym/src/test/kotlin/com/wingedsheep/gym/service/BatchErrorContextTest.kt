...
class BatchErrorContextTest : FunSpec({
    ...
    test("decision batch identifies the failing env and preserves conflict category") {
        val service = MultiEnvService(registry())
        val failing = service.create(config()).envId
        ...
        val error = shouldThrow<IllegalStateException> {
            service.submitDecisionBatch(
                listOf(
                    DecisionRequest(
                        failing,
                        YesNoResponse("no-such-decision", true)
                    )
                )
            )
        }

        error.message?.contains("decision batch item envId=$failing failed") shouldBe true
        (error.cause is IllegalStateException) shouldBe true
    }
})
