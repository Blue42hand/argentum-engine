 ...
 *
 * The sacrifice is part of resolution, not an additional cost. It therefore uses the same
 * `IfYouDo` + `PermanentsSacrificed` gate as Garruk, the Veil-Cursed: if the controller has no
 * creature to sacrifice, neither target is returned. The target requirement is one count-2 group,
 * which makes casting require two distinct creature-card targets while still letting resolution act
 * on whichever targets remain legal.
 */
